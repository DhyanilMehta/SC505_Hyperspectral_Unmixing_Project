...
def _avg_err_time_NMF(m=0, n=0, r=0, n_it=0, k=0, A=None, er_out=False):
    er = np.zeros(n_it)
    t_tot = np.zeros(n_it)
    # t_rp = np.zeros(n_it)
    W_list = [None] * n_it
...
def r_nmf(name, r, n_it, k=0, m=0, n=0, A=None, er_out=False):
    # n_r = np.size(rrange)
    # W = [None] * n_r
    # H = [None] * n_r
    # error = np.zeros(n_r)
    # t_tot = np.zeros(n_r)
    # num_iter = np.zeros(n_r)
    

    # for i in range(n_r):
    #     r = rrange[i]
    print('r = ', r)
    W_list, H_list, error, t_tot = _avg_err_time_NMF(m, n, r, n_it, k, A, er_out)
